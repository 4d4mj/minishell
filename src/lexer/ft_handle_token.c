...
/*   Updated: 2024/07/27 14:25:37 by akaterji         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_handle_token(t_data *data, int index, t_lexer **lexer_list)
{
	...
	else if (type == AND && ft_check_token(data->readline[index + 1]) == AND)
		return (ft_helper_add(AND, lexer_list));
	else if (type == SEMI && ft_check_token(data->readline[index + 1]) == SEMI)
		return (ft_helper_add(SEMI, lexer_list));
	else if (type)
	{
		if (!ft_lexer_add(NULL, type, lexer_list))
			return (-1);
